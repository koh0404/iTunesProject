...
class DesktopApp(tkinter.Frame):
    ...
    def executeProcess(self):
        ...
        url = 'https://itunes.apple.com/search?term={}&country=jp&lang=ja_jp&limit=50&media=music&entity=album&attribute=artistTerm'.format(self.text_box1.get())
        res = requests.get(url).json()
        i = 0

        with open('{}.json'.format(self.text_box1.get()), 'w', encoding="utf-8_sig") as f:
        ...
        # 縦のスクロールバー
        self.ybar = tkinter.Scrollbar(self.root_canvas, orient = 'v', command = self.root_canvas.yview)
    ...
    def SaveProcess(self, m):
        self.jpeg_high.insert(m, self.jpeg[m].replace('100x100bb.jpg', '100000x100000-999.jpg'))
        urllib.request.urlretrieve(self.jpeg_high[m], "{}_high.jpg".format(self.albumName[m])) #高画質画像取得できた！
